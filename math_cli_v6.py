...
# -*- coding: utf-8 -*-
"""
命令列版：數學練習系統（V6 主題分析報告版 - 完整修正）

更新日誌：
1. 統計強化：用 show_analysis_report 替換原有的 show_stats 和 show_recent_wrong。
2. 分析報告：按主題分類統計答對、答錯、正確率，並給予難點分析。
3. 錯題累計：顯示所有歷史累計錯題詳情。
4. 修正：確保所有核心函數 (init_db, log_record, 等) 完整且順序正確，解決所有 NameError 和 SyntaxError。
"""
