...
(function(){
  ...
  var TEMPLATE_MAP = {
    /* ------- 分數加減 ------- */
    fracAdd: {
      L1: '先判斷是不是同分母。若不同，先找公分母，再決定加分子還是減分子。\n📌 口訣：分母先統一，分子再運算。',
      L2: '💡 先列式：把每個分數改寫成同分母，再標出這一步要加還是要減。',
      L3: '📊 通分後只看分子怎麼變；分母保持不變。\n驗證：答案大小應和原本兩個分數的關係一致。',
      L4: '📝 步驟：找公分母 → 通分 → 分子加減 → 約成最簡。\n✅ 減法答案應比被減數小，最後記得檢查是否可約分。'
    },
    /* ------- 分數文字題 ------- */
    fracWord: {
    ...
    /* ------- 小數運算 ------- */
    decimal: {
      L1: '先看是乘還是除，圈出每個數有幾位小數。\n📌 先估大概範圍，再決定小數點放哪裡。',
      L2: '💡 先列式：暫時把小數看成整數算，旁邊記下總共有幾位小數。',
      L3: '📊 乘法看兩數小數位數總和；除法遇到不夠除就補 0，商的小數點要對齊。\n驗證：答案量級不能差太多。',
      L4: '📝 先算整數部分 → 再放回小數點。\n✅ 用估算檢查：乘以大於 1 會變大，乘以小於 1 會變小。'
    },
    /* ------- 百分率 / 折扣 ------- */
    percent: {
      L1: '先圈出誰是全體、誰是部分、誰是百分率。\n📌 % 先改成 ÷100 或小數，再決定要求部分、全體還是比率。',
      L2: '💡 先列式：求部分用 全體×百分率；求全體用 部分÷百分率。',
      L3: '📊 打折、加成、稅金都先找倍率，再看這一步要乘還是要除。\n驗證：打折後應變小，加成後應變大。',
      L4: '📝 步驟：百分率↔小數 → 代入公式 → 檢查單位。\n✅ 若答案超過 100%，先確認題意是不是成長或超過原量。'
    },
    /* ------- 時間 ------- */
    time: {
      L1: '先把時間統一成時分或全部換成分鐘。\n📌 注意有沒有跨整點、跨中午或跨日。',
      L2: '💡 先列式：加法看滿 60 進 1 小時；減法看不夠減要先借 60 分。',
      L3: '📊 可以先用分鐘算完，再換回時分。\n驗證：結束時間應和起始時間方向一致，不要早於起點。',
      L4: '📝 步驟：統一單位 → 運算 → 轉回題目要求格式。\n✅ 最後把答案放回鐘面或時間線上檢查是否合理。'
    },
    /* ------- 體積 / 面積 ------- */
    volume: {
      L1: '先圈出長、寬、高與單位，先判斷求面積還是體積。\n📌 體積要用立方單位，複合形體先想怎麼分塊。',
      L2: '💡 先列式：底面積 = 長×寬；體積 = 底面積×高。',
      L3: '📊 複合形體先拆成幾個基本形，各自算完再相加。\n驗證：cm³、m³ 轉換時要先看是不是體積換算。',
      L4: '📝 步驟：判斷單位 → 列底面積 → 乘高或分塊相加。\n✅ 若答案太大或太小，先回頭檢查單位是否用錯。'
    },
    /* ------- 平均 ------- */
    average: {
      L1: '先圈出總量和分成幾份，確認要找平均數還是平均單價。\n📌 平均一定是「總量 ÷ 份數」。',
      L2: '💡 先列式：若題目分兩步，先把總量湊齊，再除以份數。',
      L3: '📊 驗證時把平均×份數，看能不能回到原本總量。\n答案通常會落在最大值和最小值之間。',
      L4: '📝 步驟：先求總量 → 再除以份數 → 檢查單位。\n✅ 平均單價要寫成每個、每份、每瓶這類單位。'
    },
    /* ------- 數論（因數/倍數/質合數/倒數） ------- */
    numberTheory: {
      L1: '先判斷題目要找因數、倍數、最大公因數還是最小公倍數。\n📌 因數看能不能整除，倍數看要乘幾次。',
      L2: '💡 先列式：把數字分解或列出倍數，再找共同部分。',
      L3: '📊 GCD 看共同因數中最大的一個；LCM 看共同倍數中最小的一個。\n驗證：GCD 要能整除原數，LCM 要同時被原數整除。',
      L4: '📝 步驟：列因數/倍數或做質因數分解 → 找共同部分 → 寫答案。\n✅ 分裝或平均分組多半偏向 GCD；同時週期多半偏向 LCM。'
    },
    /* ------- 統計圖表 ------- */
    dataStats: {
      L1: '先看標題、單位和橫軸縱軸，確認要找最大、最小、差或趨勢。\n📌 先讀圖，再動手算。',
      L2: '💡 先列式：把要比較的兩個點或幾個資料先讀成數字。',
      L3: '📊 折線圖看上升下降；長條圖直接比高低；表格先抓同一欄或同一列。\n驗證：單位要和圖表一致。',
      L4: '📝 步驟：讀值 → 比較/加減/平均 → 回答題目。\n✅ 若題目問「增加多少」，要用相減，不是直接抄較大的值。'
    },
    /* ------- 幾何性質（對稱/垂直/角度） ------- */
    geometry: {
      L1: '先圈出圖形名稱、已知邊長或角度，判斷要用哪個性質。\n📌 對稱看左右對應，垂直看 90°，圓心角看整個圓。',
      L2: '💡 先列式：把已知量標到圖上，再決定這一步要加、減還是平分。',
      L3: '📊 從圖上讀出對應邊、對應角或對稱軸數。\n驗證：角度總和或圖形性質要說得通。',
      L4: '📝 步驟：標圖 → 套性質 → 算未知量。\n✅ 算完回頭看圖，答案要和位置與大小感覺一致。'
    },
    /* ------- 位值與大數比較 ------- */
    placeValue: {
      L1: '先從最高位往右看，圈出題目指定的是哪一位。\n📌 位值 = 這個數字所在的位置大小。',
      L2: '💡 先列式：寫成位值表或展開式，再做比較、取概數或判斷 digit。',
      L3: '📊 比大小要先比最高位；四捨五入要看右邊一位。\n驗證：萬、億這類單位不要看錯位。',
      L4: '📝 步驟：找指定位 → 寫出位值/展開式 → 再比較或取概數。\n✅ 若答案差很多，先檢查是不是少看一個 0。'
    },
    /* ------- 簡易方程式 ------- */
    simpleEquation: {
      L1: '先圈出未知數和等號兩邊，確認 x 前面是加、減、乘還是除。\n📌 解方程式就是用逆運算把 x 單獨留下。',
      L2: '💡 先列式：想像天平平衡，左右兩邊要做相同動作。',
      L3: '📊 加減用相反運算，乘除也用相反運算。\n驗證：解完把 x 代回原式，左右應相等。',
      L4: '📝 步驟：移除跟 x 黏在一起的數 → 求出 x → 代回檢查。\n✅ 若代回不相等，通常是正負號或乘除順序看錯。'
    },
    /* ------- 通用兜底 ------- */
    generic: {
      L1: '先圈出題目的已知量、未知量與單位。\n📌 想好「對誰做運算」。',
