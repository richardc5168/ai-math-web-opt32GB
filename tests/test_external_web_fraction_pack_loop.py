...
@pytest.mark.anyio
async def test_external_web_fraction_pack_loop(tmp_path):
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        response = await client.post(
            "/v1/questions/next",
            params={"student_id": student_id, "topic_key": "external_web_fraction_app_v1"},
            headers=headers,
        )
        assert response.status_code == 200
        ...
        assert data.get("topic") == "external_web_fraction_app_v1"
